/**
 * Claritromicina 125 mg/5 mL - Suspensão Oral
 */

import { MedicamentoData, MedicamentoExport, ResultadoCalculo } from '../../types';

export const data: MedicamentoData = {
  id: 'claritromicina-125mg-5ml',
  nome: 'Claritromicina 125 mg/5 mL',
  ...
  parametrosCalculo: {
    ...
    formulaCalculo: 'Peso (kg) × 0,3 mL = "mL" de 12/12h',
  },
  restricaoIdade: {
    idadeMinima: '≥ 3 meses',
  },
  observacoes: 'Pode ser tomado com ou sem alimentos.',
  alertas: ['Pode prolongar intervalo QT', 'Interações medicamentosas importantes'],
  ultimaAtualizacao: '2026-02-02',
};

export function calcularDose(peso: number): ResultadoCalculo {
  const dosePorTomadaMg = Math.min(peso * 7.5, 500);
  const volumeMl = (dosePorTomadaMg / 125) * 5;
  const volumeArredondado = Math.round(volumeMl * 10) / 10;

  const alertas: string[] = [];
  if (peso * 15 >= 1000) alertas.push('Dose máxima diária atingida');
